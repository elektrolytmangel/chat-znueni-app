...
const CustomRoleForm = (props: Props) => {
  ...
  useEffect(() => {
    if (customContext || origin || fullName) {
      var preview = getSystemContent({ customContext, origin, fullName, context: "custom", displayName: fullName } as AiRole);
      ...
      <Modal show={show} className='mt-5'>
        <Modal.Body >
          <form className='d-grid p-2 gap-2' onSubmit={handleSubmit((data) => onHandleSubmit(data))}>
            ...
            <input id="name_field" className='input-style' placeholder={t('placeholder_name') || ''} {...register('fullName', { required: true, })} />
            <label htmlFor="origin_field">{t('origin')} *</label>
            <input id="origin_field" className='input-style' placeholder={t('placeholder_origin') || ''} {...register('origin', { required: true })} />
            <label htmlFor="context_field">{t('context')}</label>
            <textarea id="context_field" className='input-style' placeholder={t('placeholder_context') || ''} {...register('customContext')} />
